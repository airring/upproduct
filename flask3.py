...
def pro_static():
	if 'username' in session:
		up_service_name = request.form.get('service_name')
		print(up_service_name)
		if up_service_name:
			db_logs = t_log.query.filter(t_log.pro_name == up_service_name ).order_by(t_log.update_time.desc()).limit(200).all()
		else:
			db_logs = t_log.query.order_by(t_log.update_time.desc()).limit(200).all()
		...
		return render_template('pro_static.html' ,db_logs=db_logs,username=session['username'])
...
def proshenji():
	if 'username' in session:
		...
		for i in request.form.getlist('service_choies'):
			...
			# 	static.uppro_time = int(time.time())
			# 	i['build_static'] = u'%s 无需发布生产' % static.pro_name
			# 	static.static = 15
			# elif static.pro_name in ArgInput.branch_arg:
			...
			db.session.commit()
			build_server.append(i)
		return render_template('shenji.html' , static=build_server,username=session['username'])
...
def checkshenji():
    if 'username' in session:
        ...
        build_server=[]
        serverices_name = request.form.getlist('service_choies')
        up_note = request.form.get('note')
        for i in serverices_name:
            a={}
            static = t_log.query.filter(t_log.id == i).first()
            ...
            static.update_time = int(time.time())
            db.session.commit()
            a['build_static'] = u'%s 已经验收通过，等待审批上线' % static.pro_name
            build_server.append(a)
    return render_template('shenji.html', static=build_server)
...
def testbuild():
	if 'username' in session:
		...
		return render_template('build.html' ,db_logs=db_logs,html_page='shenji.html',username=session['username'])
...
def checkok():
    ...
    if 'username' in session:
        ...
        return render_template('build.html' ,db_logs=db_logs,html_page='checkshenji.html',username=session['username'])
...
def releaseok():
    ...
    if 'username' in session:
        ...
        db_logs = t_log.query.filter(t_log.static == 5).all()
